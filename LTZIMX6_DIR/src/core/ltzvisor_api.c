...
extern uint32_t ns_vectors;
extern uint32_t DONE_START_NS;
/*
...
extern void vectors_secondary();
extern void ns_monitor ();
extern void world_first_context_save(tzmachine*);
/**
...
uint32_t ltzvisor_nsguest_create( struct nsguest_conf_entry *g )
{
	...
	NS_Guest.core.vcpu_regs_core.spsr_mon = 0x13;
	...
	NS_Guest.core.vcpu_regs_cp15.c12_VBAR = &ns_vectors;
	printk("      * NS_Guest CP15 VBAR: %x - OK  \n\t", &ns_vectors);
...
//	#endif
	...
	//world_first_context_save((uint32_t)&NS_Guest);
...
void multicore_cp15_restore(void){
		/* Get NS_Guest ready to run */
 		cp15_restore(&NS_Guests[0].core.vcpu_regs_cp15);
}



uint32_t ltzvisor_CORE_nsguest_create( struct nsguest_conf_entry *g ){
		int a = 0;
		for(a = 0; a < CORE_NS_GUEST; a++){
			  /** Init Guest attributes */
				NS_Guests[a].booting = 1;
				NS_Guests[a].id = g->gce_id;
				strcpy(NS_Guests[a].name, g->gce_name);
				printk("      * NS_Guest attributes - OK  \n\t");

				/** Guest Core Registers */
				/* Clean core registers */
				memset(&NS_Guests[a].core.vcpu_regs_core,0,sizeof(struct core_regs));
				/* lr = start_addr & spsr=supervisor */
				NS_Guests[a].core.vcpu_regs_core.lr_mon = g->gce_bin_load;
				NS_Guests[a].core.vcpu_regs_core.spsr_mon = 0x193;
				printk("      * NS_Guest core registers - OK  \n\t");

				/** Guest CP15 Registers */
				/* Clean CP15 registers */
				memset(&NS_Guests[a].core.vcpu_regs_cp15,0,sizeof(struct cp15_regs));
				NS_Guests[a].core.vcpu_regs_cp15.c1_SCTLR = 0x00c50078;

				printk("      * NS_Guest CP15 registers - OK  \n\t");

			//	#ifdef CONFIG_CACHE_L2X0
					NS_Guests[a].core.vcpu_regs_cp15.c1_ACTLR = 0x00000006; /* L1 prefetch enable -bit2- + L2 Prefetch hint enable -bit1-*/
					printk("      * NS_Guest L2 Cache - OK  \n\t");
			//	#endif
				...
				printk("      * NS_Guest gce_bin_start: 0x%x \n\t", g->gce_bin_start);
				printk("      * NS_Guest gce_bin_end: 0x%x \n\t", g->gce_bin_end);
			//	memset((uint32_t *)g->gce_bin_load,(uint32_t *)g->gce_bin_start,(g->gce_bin_end - g->gce_bin_start));
			//	if(g->gce_trd_init) {
			//		memset((uint32_t *)g->gce_trd_load,(uint32_t *)g->gce_trd_start,(g->gce_trd_end - g->gce_trd_start));
			//	}
			/*	memcpy((uint32_t *)g->gce_bin_load,(uint32_t *)g->gce_bin_start,(g->gce_bin_end - g->gce_bin_start));
				if(g->gce_trd_init) {
					memcpy((uint32_t *)g->gce_trd_load,(uint32_t *)g->gce_trd_start,(g->gce_trd_end - g->gce_trd_start));
				...
				/* Get NS_Guest ready to run */
				//cp15_restore(&NS_Guests[a].core.vcpu_regs_cp15);
				printk("      * NS_Guest%d created - OK  \n\t",a);

				printk("      * NS_Guest Context Address: %x \n\t",(uint32_t)&NS_Guests[a]);
				g++;
    }
		set_guest_context( (uint32_t)&NS_Guests[0]);
		printk("      * NS_Guest Context Address: %x \n\t",sizeof(NS_Guests));
		printk("      * NS_Guests ready to run - OK  \n\t",a);

		printk ("Context Struct CP15_SHIFT: %x\n ", SHIFT_CP15);
		printk ("Sizeof Context Struct :%x\n ", CONTEXT_SHIFT);
...
void set_VBAR(uint32_t addr) {
	...
	void set_MVBAR(uint32_t addr) {
		/** Software Thread ID - TPIDRPRW */
		asm volatile("mcr	p15, 0, r0, c12, c0, 0\n\t");
}

void ltzvisor_start_core_guest(tzmachine *NSG_ptr){
		void *funcptr(void* );
		 int i = 0;
		 for (i = 0 ; i < CORE_NS_GUEST; i++){
			 ltzvisor_spin_lock(&printk_spinlock, TIMEOUT_FOREVER);
			 	printk("CORE%d Starting Guest%d\n",cpu_get_current(), i+1);
				ltzvisor_spin_unlock(&printk_spinlock);
				if (i > 0){
					cp15_save(&NS_Guests[i-1].core.vcpu_regs_cp15);
					cp15_restore(&NS_Guests[i].core.vcpu_regs_cp15);
					set_VBAR(vectors_secondary);
					set_MVBAR(ns_monitor);
				}
 				start_ns_guest((void*)NS_Guests[i].core.vcpu_regs_core.lr_mon);
				if(i == CORE_NS_GUEST-1)
						DONE_START_NS = 0xFF;
		 }
